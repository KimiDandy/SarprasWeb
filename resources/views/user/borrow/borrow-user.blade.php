...
@section('content')
                ...
                <div class="card custom-card">
                    <div class="card-header">
                        ...
                        <div class="mb-3 d-flex align-items-center justify-content-center">
                            <button type="button" id="addBarang" class="btn btn-primary"><i class="fas fa-plus"></i> Tambah Barang</button>
                        </div>
                    </div>
                    <hr class="m-0" style="opacity: 30%; height: 0.7px;">
                    <div class="card-body">
                        <div class="basic-form">
                            <form id="pinjamBarangForm" method="post" action="{{ route('peminjaman.store') }}" class="form-valide-with-icon needs-validation" novalidate>
                                @csrf

                                <div id="formContainer">
                                    <div class="row mb-5">
                                        <div class="col mt-2 mt-sm-0" id="borrowdate">
                                            <label class="text-label form-label ps-2" style="font-size: 15px; font-weight: 500">Tanggal Pinjam</label>
                                            <div class="input-group" data-placement="left" data-align="top" data-autobtn-close="true">
                                                <span class="input-group-text"><i class="fas fa-calendar"></i></span>
                                                <input type="date" id="borrow" class="form-control input-default custom-border" name="tanggal_pinjam" required>
                                            </div>
                                        </div>
                                        <div class="col mt-2 mt-sm-0" id="returndate">
                                            <label class="text-label form-label ps-2" style="font-size: 15px; font-weight: 500">Tanggal Kembali</label>
                                            <div class="input-group" data-placement="left" data-align="top" data-autobtn-close="true">
                                                <span class="input-group-text"><i class="fas fa-calendar"></i></span>
                                                <input type="date" class="form-control input-default custom-border" name="tanggal_kembali" required>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <button type="button" class="btn btn-success d-flex justify-content-end align-items-end text-end" id="submitBtn">Simpan</button>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            addButton.addEventListener('click', function() {
                ...
                barangList.forEach(function(barang) {
                    options += `<option value="${barang.id}">${barang.nama_barang}</option>`;
                });
                ...
                <div class="col-10">
                    <div class="form-unit form-divided mb-3" style="border-radius: 1px">
                        ...
                        <select class="js-equip custom-border" name="barang[${equipCount}][id_barang]" required>
                            ${options}
                        </select>
                    </div>
                </div>
                <div class="col-2" style="height: 30px">
                    ...
                    <input type="number" class="form-control input-default custom-border" placeholder="Jumlah" name="barang[${equipCount}][jumlah]" min="1" required>
                </div>
            ...
            function validateForm() {
                ...
                const returnDate = document.getElementsByName('tanggal_kembali')[0].value.trim();
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            var returnInput = document.getElementsByName('tanggal_kembali')[0];
            ...
            $(".js-equip").select2({
                ...
                if ($(this).val() && $(this).val().length) {
                    $(this).next('.select2-container')
                        .find('li.select2-search--inline input.select2-search__field').attr('placeholder', 'Barang');
                }
            });
        });
    </script>
